package org.projedata.industry.configuration;

import org.springdoc.core.customizers.OpenApiCustomiser;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import io.swagger.v3.oas.models.ExternalDocumentation;
import io.swagger.v3.oas.models.OpenAPI;
import io.swagger.v3.oas.models.info.Contact;
import io.swagger.v3.oas.models.info.Info;
import io.swagger.v3.oas.models.info.License;
import io.swagger.v3.oas.models.responses.ApiResponse;
import io.swagger.v3.oas.models.responses.ApiResponses;

@Configuration
public class SwaggerConfig {

	@Bean	
	public OpenAPI springIndustryOpenAPI() {
		return new OpenAPI()
				.info(new Info()
					.title("Industry")
					.description("Industry - Projedata")
					.version("v0.0.1")
				.license(new License()
					.name("projedata.com.br")
					.url("http://springdoc.org"))
				.contact(new Contact()
					.name("Daniel Reis")
					.url("https://www.linkedin.com/in/daniel-reis-39572b222/")
					.email("kr.danielreis@gmail.com")))
				.externalDocs(new ExternalDocumentation()
					.description("Github")
					.url("https://github.com/DanielSantosReis/IndustryProject"));
	}
	
	@Bean
	public OpenApiCustomiser customerGlobalHeaderOpenApiCustomiser() {

		return openApi -> {
			openApi.getPaths().values().forEach(pathItem -> pathItem.readOperations().forEach(operation -> {

				ApiResponses apiResponses = operation.getResponses();

				apiResponses.addApiResponse("200", createApiResponse("Success"));
				apiResponses.addApiResponse("201", createApiResponse("Created"));
				apiResponses.addApiResponse("204", createApiResponse("No Content"));
				apiResponses.addApiResponse("400", createApiResponse("Bad Request"));
				apiResponses.addApiResponse("401", createApiResponse("Unauthorized"));
				apiResponses.addApiResponse("404", createApiResponse("Not Found"));
				apiResponses.addApiResponse("500", createApiResponse("Internal Server Error"));

			}));
		};
	}

	private ApiResponse createApiResponse(String message) {

		return new ApiResponse().description(message);

	}

}
